...
export function StoreRequestsPage() {
  ...
  function effectiveStatus(id: string, seed: ApplicationStatus): ApplicationStatus {
  ...
  const appsWithStatus = apps.map(a => ({ ...a, status: effectiveStatus(a.id, a.status) }))
  const filtered = tab === 'all' ? appsWithStatus : appsWithStatus.filter(a => a.status === tab)
  const pendingCount = appsWithStatus.filter(a => a.status === 'pending').length
                    ...
                    {app.status === 'pending' && (
                      <div className="flex gap-1">
                        <button
                          ...
                          className="text-[10px] text-green border border-green/30 rounded-[4px] px-[7px] py-[2px] bg-transparent cursor-pointer hover:bg-green/5 transition-colors"
                        >
                          Approve
                        </button>
                        <button
                          onClick={() => handleDecision(app.id, 'rejected')}
                          className="text-[10px] text-red border border-red/20 rounded-[4px] px-[7px] py-[2px] bg-transparent cursor-pointer hover:bg-red/5 transition-colors"
                        >
                          Reject
                        </button>
                      </div>
