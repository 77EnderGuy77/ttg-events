  ...
  return (
    <div className="p-6">
      ...
      <div className="border-b border-line mb-5 flex gap-0">
        <TabBtn id="registrations" active={tab} onClick={setTab}>Registrations ({registered.length})</TabBtn>
        <TabBtn id="checkin" active={tab} onClick={setTab}>Check-in</TabBtn>
        <TabBtn id="waitlist" active={tab} onClick={setTab}>Waitlist ({waitlisted.length})</TabBtn>
        <TabBtn id="announcements" active={tab} onClick={setTab}>Announcements ({announcements.length})</TabBtn>
      </div>
                ...
                <TableRow key={r.id}>
                  ...
                  <TableCell>
                    <div className="flex gap-1.5">
                      {!cancelled.has(r.id) && (
                        <Button size="xs" variant={checkedIn.has(r.id) ? 'success' : 'ghost'} onClick={() => handleCheckIn(r.id)}>
      ...
      {tab === 'checkin' && (
        <div>
          <p className="text-[12px] text-ink-3 mb-4">{checkedIn.size} / {registered.length} checked in this session</p>
              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-between px-4 py-3 rounded-[8px] border transition-colors ${
                    isIn ? 'bg-green/10 border-green/30 text-green' : 'bg-surface-2 border-line text-ink hover:border-gold/40'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center text-[10px] ${isIn ? 'border-green bg-green text-surface' : 'border-line'}`}>
                      {isIn && '✓'}
                    </div>
              ...
              <TableBody>
                {waitlisted.map((r, idx) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell>
                      <div className="flex gap-1.5">
                        <Button size="xs" variant="success" onClick={() => handlePromote(r.id)}>Promote</Button>
                        <Button size="xs" variant="danger" onClick={() => handleCancel(r.id)}>Remove</Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
