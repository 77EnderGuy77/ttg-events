      ...
      <div className="flex flex-wrap gap-2 mb-5">
        ...
        <button
          ...
          className={`text-[11px] px-3 py-1.5 rounded-[6px] border transition-colors ${
            gameFilter === 'all' ? 'bg-purple/10 text-purple border-purple/30' : 'bg-surface-2 text-ink-3 border-line hover:text-ink'
          }`}
        >
          All games
        </button>
        {games.map(g => (
          <button
            ...
            className={`text-[11px] px-3 py-1.5 rounded-[6px] border transition-colors ${
              gameFilter === g ? 'bg-purple/10 text-purple border-purple/30' : 'bg-surface-2 text-ink-3 border-line hover:text-ink'
            }`}
          >
            {gameLabel(g)}
          </button>
      ...
      {isLoading ? (
        ...
        <div className="bg-surface-2 border border-line rounded-[10px] overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {sorted.map(ev => {
                const filled = getConfirmedCount(ev.id)
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-16 h-[3px] bg-surface-4 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${pct}%`, background: pct >= 100 ? 'var(--red)' : pct >= 75 ? 'var(--gold)' : 'var(--green)' }}
                          />
                        </div>
